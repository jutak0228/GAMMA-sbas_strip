...
# average backscatter
width=`cat ${work_dir}/DEM_prep/${dem_name}.dem_par | grep "width" | awk -F":" '{print $2}' | tr -d [:space:] | sed -e "s/[^0-9]//g"`
nlines=`cat ${work_dir}/DEM_prep/${dem_name}.dem_par | grep "nlines" | awk -F":" '{print $2}' | tr -d [:space:] | sed -e "s/[^0-9]//g"`
...
data2geotiff ${work_dir}/DEM_prep/${dem_name}.dem_par geo/${dem_name}.los_def_rate.bmp 0 geo/${dem_name}.los_def_rate.tif
# ...

# generate geotiff files (float values)
data2geotiff ${work_dir}/DEM_prep/${dem_name}.dem_par geo/${dem_name}.los_def_rate 2 geo/${dem_name}.los_def_rate.float.tif
# generate geotiff files (float values) for each disp_date
run_all dates_geoback 'data2geotiff ../DEM_prep/$2.dem_par geo/$2.$1.disp 2 geo/$1.$2.float.tif'
...
kml_map geo/${dem_name}.los_def_rate.png ${work_dir}/DEM_prep/${dem_name}.dem_par geo/${dem_name}.los_def_rate.kml
